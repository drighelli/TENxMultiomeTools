...
Describe Multiome data

highlight that this is a pipeline intended for differential analysis of multiome data
...
Because the data are not published yet, we are using a publicly available dataset
to illustrate the available functionalities of the package.
...
+ data: data type to load, filtered or raw
...
+ addfrags: logical indicating if the path to the frag file has to be saved in 
the sce. Note that this is needed when computing the metrics in the next step.
...
information about the peaks in a GenomicRanges format where the genomic coordinates 
are in the form of Chromosome, Start and End while additional data are stored in
the mcols DataFrame.
...
We implemented a wrapped on the `scDblFinder` package, 
...
But to better understand the doublets we suggest to assign cell labels first and
then to investigate where the doublets fall in our dataset.
...
available from the AllenInstituteBrainData package (https://github.com/drighelli/AllenInstituteBrainData) 
...
After downloading the dataset, it could be useful to create pseudobulk counts
to speed up our computations.
Using a reference of 1 Million cells could take very long time during the process
of cell type annotation, and from our tests using the pseudobuld counts is a 
...
```{r}
tsne <- plotTSNE(multiome[[n]],colour_by="SingleR")+
...
A possible approach to obtain pseudobulk data from your single cell is to 
...
In the end we'll have a new SingleCellExperiment with a reduced number of 
columns, which, in turn, will be the same number of cell types defined in our
SingleR colData column.

```{r}
sce_pseudo <- aggregateAcrossCells(sce, use.assay.type="counts",
...
But when we have multiple ATAC experiments, coming from different conditions,
it could be problematic to compute a pseudo-bulk counts, because of the 
differences across the peaks.

At the moment we are implementing (still needing some bug fixing and testing) a 
solution to this problem by adapting the `finalRegions` function present in the 
DEScan2 package, which will return a new SingleCellExperiment object with the 
count matrix computed on the multiple experiments.

Both previous operations on a single experiment can be made with our wrapper 
`computePseudoBulk` in a unique instruction that computes pseudobulk counts
both for RNA and ATAC assays.

```{r, eval=FALSE}
sce_pseudo <- computePseudoBulk(sce, ctCol="SingleR")
```

At this point is easy to apply standard bulk pipelines to obtain differentially
expressed genes and differentially accessible regions, per each cell type across
